...
function starterScene(template='starter-3d') {
  ...
  if (template === 'starter-3d') {
    ...
    const block = baseObject('box', 'Scale Reference Block', [7,6.05,4], { id:'block-main', scale:[6,4,6], properties:{ color:'#3f66a3',metallic:.08,roughness:.78,collider:true,editorReference:true,renderClass:'editor-only',affectsSurfaceRecipes:false,purpose:'Starter scale, collision, lighting, and shadow reference. No scene-management behavior.' } });
    const marker = baseObject('sphere', 'World Marker', [-12,4,-4], { id:'marker-main', scale:[2.2,2.2,2.2], properties:{ color:'#a96cff',metallic:.2,roughness:.42,collider:true,editorReference:true,renderClass:'editor-only',affectsSurfaceRecipes:false,purpose:'Starter world-position reference. Not included in game-accurate captures or surface placement rules.' } });
...
function migrateState(input) {
  ...
  for (const scene of state.scenes || []) {
    scene.settings = { fogNear:90,fogFar:280,exposure:1,viewportLightingMode:'authoring-assist',gridVisible:true,splinesVisible:true,worldChunkSize:64,waterLevel:-100,windDirection:[1,0,.25],windStrength:.35,season:'summer',weatherWetness:0,weatherSnow:0,...scene.settings };
    migrateSceneWorldFoundation(scene);
    const referenceBlock=scene.objects.find(object=>object.id==='block-main');if(referenceBlock){referenceBlock.name='Scale Reference Block';referenceBlock.properties={...(referenceBlock.properties||{}),editorReference:true,renderClass:'editor-only',affectsSurfaceRecipes:false,purpose:'Starter scale, collision, lighting, and shadow reference. No scene-management behavior.'};}
    const worldMarker=scene.objects.find(object=>object.id==='marker-main');if(worldMarker)worldMarker.properties={...(worldMarker.properties||{}),editorReference:true,renderClass:'editor-only',affectsSurfaceRecipes:false,purpose:'Starter world-position reference. Not included in game-accurate captures or surface placement rules.'};
