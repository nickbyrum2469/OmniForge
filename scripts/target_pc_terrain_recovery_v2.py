...
PATH_VISUALS = r'''import { normalizePathProperties, normalizeTerrainProperties, samplePathSpline, terrainHeightAt } from './worldgen.js';
...
def apply(root: Path, changed: list[str]) -> None:
    ...
    if 'pathSurfaceFor(pathObject,scene)' not in text:
        anchor = "  cameraMatrices(camera){"
        if anchor not in text:
            raise RuntimeError('Renderer path-buffer insertion anchor was not found.')
        methods = """  pathSurfaceFor(pathObject,scene){
...
"""
        renderer.write_text(text.replace(anchor, methods + anchor, 1), encoding='utf-8')
        changed.append(renderer.relative_to(root).as_posix())
    ...
    text = renderer.read_text(encoding='utf-8')
    insertion = "      if(object.type==='terrain')this.renderPathSurfacePass(frame);\n"
    draw_line = "      this.drawMesh(object,mesh,viewProj,lightViewProj,scene,object.id===selectedId,camera,lights);\n"
    if insertion not in text:
        if draw_line not in text:
            raise RuntimeError('Opaque terrain draw insertion anchor was not found.')
        renderer.write_text(text.replace(draw_line, draw_line + insertion, 1), encoding='utf-8')
        changed.append(renderer.relative_to(root).as_posix())

    text = renderer.read_text(encoding='utf-8')
    if '  renderPathSurfacePass(frame){' not in text:
        anchor = "  renderEditorOverlayPass(frame){"
        if anchor not in text:
            raise RuntimeError('Editor overlay pass insertion anchor was not found.')
        method = """  renderPathSurfacePass(frame){
...
"""
        renderer.write_text(text.replace(anchor, method + anchor, 1), encoding='utf-8')
        changed.append(renderer.relative_to(root).as_posix())
