...
function replaceRequired(source, before, after, path) {
  ...
  if (!source.includes(before)) throw new Error(`Expected block was not found in ${path}: ${before.slice(0, 100)}`);
...
edit('server/v010-api.mjs', source => source.replace(
...
`  const angle = ((hour - 6) / 24) * Math.PI * 2;
  ...
  const day = clamp((elevation + 0.08) / 0.32, 0, 1);`, 'server/v010-systems.mjs');
  next = replaceRequired(next,
`  const azimuth = (hour / 24) * 360 - 90;
  sun.name = 'Sun';
  sun.transform.rotation = [90 - elevation * 82, azimuth, 0];`,
`  sun.name = 'Sun';
  sun.transform.rotation = [90 - sunElevationDegrees, sunAzimuth, 0];`, 'server/v010-systems.mjs');
  next = replaceRequired(next,
`  };
...
`  };
  ...
  if (!moon) {
    moon = {
      ...
      locked: false,
      parentId: null,
      transform: { position: [0, 0, 0], rotation: [58, 90, 0], scale: [1, 1, 1] },
      properties: {},
      components: []
    };
...
`    <div class="v010-card">
      ...
      <p class="v010-section-note">The Sun and Moon are authoritative celestial bodies. Time-driven mode follows world time; Manual mode preserves the azimuth and elevation entered here.</p>
    </div>

    <div class="v010-card">
      <header><b>Atmosphere Studio</b><span class="v010-chip">SCATTERING + AERIAL PERSPECTIVE</span></header>`, 'app/v010.js');
  next = replaceRequired(next,
...
`        <label>Exposure<input id="v010Exposure" type="range" min="0.2" max="3" step="0.02"></label>
        ...
        <label>Fog<input id="v010Fog" type="range" min="0" max="1" step="0.01"></label>`, 'app/v010.js');
  next = replaceRequired(next,
...
`  field('v010Humidity').value = world.atmosphere.humidity;
  ...
  const species = assets.filter(item => item.type === 'foliageSpecies');`, 'app/v010.js');
  next = replaceRequired(next,
...
edit('app/worldgen.js', source => {
  let next = source;
  if (!next.includes('function normalizeSculptLayer')) {
    next = next.replace('function presetKey(value) {', `function normalizeSculptLayer(layer = {}) {
  return {
...
`    expandStep: clamp(properties.expandStep ?? 100, 1, 10000),
    ...
    densityLimited: Boolean(properties.densityLimited),`, 'app/worldgen.js');
  next = replaceRequired(next,
...
`    showSpline: properties.showSpline !== false,
    ...
    carveTerrain: Boolean(properties.carveTerrain),`, 'app/worldgen.js');
  next = replaceRequired(next,
...
`export function expandTerrain(terrain, direction, amount) {
  ...
  const delta = Math.max(1, Number(amount || properties.expandStep || 100));
  const bounds = { ...properties.bounds };`, 'app/worldgen.js');
  next = replaceRequired(next,
...
`  const requiredResolutionX = Math.ceil((bounds.maxX - bounds.minX) / Math.max(EPSILON, spacingX));
  ...
  }, { ...terrain.transform, scale: [1, 1, 1] });`, 'app/worldgen.js');
  if (!next.includes('export function addTerrainSculptLayer')) {
    next = next.replace('export function migrateSceneWorldFoundation(scene) {', `export function addTerrainSculptLayer(terrain, layer = {}) {
  const properties = normalizeTerrainProperties(terrain.properties || {}, terrain.transform || {});
...
edit('server/v011-systems.mjs', source => {
  let next = source;
  next = next.replace('  splitPath\n} from \'../app/worldgen.js\';', "  splitPath,\n  addTerrainSculptLayer,\n  undoTerrainSculpt,\n  clearTerrainSculpt\n} from '../app/worldgen.js';");
  next = next.replace('export { expandTerrain, insertPathPoint, splitPath, normalizeTerrainProperties, normalizePathProperties, migrateSceneWorldFoundation, terrainHeightAt, terrainNormalAt, samplePathSpline };', 'export { expandTerrain, insertPathPoint, splitPath, addTerrainSculptLayer, undoTerrainSculpt, clearTerrainSculpt, normalizeTerrainProperties, normalizePathProperties, migrateSceneWorldFoundation, terrainHeightAt, terrainNormalAt, samplePathSpline };');
  return next;
});

edit('server/v011-api.mjs', source => {
  let next = source;
  next = next.replace('  splitPath,\n  normalizePathProperties,', '  splitPath,\n  addTerrainSculptLayer,\n  undoTerrainSculpt,\n  clearTerrainSculpt,\n  normalizePathProperties,');
  if (!next.includes("/sculpt/undo")) {
    next = next.replace(`    ids = match(url.pathname, /^\\/api\\/v011\\/path\\/([^/]+)$/);`, `    ids = match(url.pathname, /^\\/api\\/v011\\/terrain\\/([^/]+)\\/sculpt$/);
...
`        const path = requirePath(state, ids[0]);
        ...
        updatePathProperties(path, { points: inserted.points });`, 'server/v011-api.mjs');
  return next;
});

edit('app/v011.js', source => {
  let next = source;
  next = replaceRequired(next,
...
let draggingNode = null;`, 'app/v011.js');
  next = replaceRequired(next,
...
`  const diagnostics = foundation?.terrainDiagnostics;
  ...
  return \`<section class="v011-authoring-panel" data-v011-panel="terrain">`, 'app/v011.js');
  next = replaceRequired(next,
...
`    <div class="v011-sculpt-controls">
    ...
    <div class="v011-readout"><span>Bounds</span><code>${properties.bounds ? \`${properties.bounds.minX.toFixed(0)}, ${properties.bounds.minZ.toFixed(0)} → ${properties.bounds.maxX.toFixed(0)}, ${properties.bounds.maxZ.toFixed(0)}\` : 'not migrated'}</code></div>
    <div class="v011-readout"><span>Mesh density</span><code>${spacingX.toFixed(2)} × ${spacingZ.toFixed(2)} m/vertex · ${densityStatus}</code></div>`, 'app/v011.js');
  next = replaceRequired(next,
...
`  const diagnostics = foundation?.pathDiagnostics?.find(item => item.pathId === object.id);
  ...
  const selectedPoint = properties.points?.[selectedIndex] || [0, 0];`, 'app/v011.js');
  next = replaceRequired(next,
...
`      ${numberControl('Width', 'width', properties.width ?? 3, { step: 0.25, min: 0.1, max: 200 })}
      ...
      ${numberControl('Spline tension', 'splineTension', properties.splineTension, { step: 0.05, min: 0, max: 1 })}
      ${numberControl('Samples/segment', 'samplesPerSegment', properties.samplesPerSegment, { step: 1, min: 2, max: 64 })}`, 'app/v011.js');
  next = replaceRequired(next,
...
`    <div class="v011-node-editor">
    ...
    <div class="v011-actions"><button id="v011ReversePath" type="button">Reverse direction</button></div>`, 'app/v011.js');
  next = replaceRequired(next,
...
`  container.querySelectorAll('[data-v011-expand]').forEach(button => button.addEventListener('click', () => expandWorld(object.id, button.dataset.v011Expand)));
  ...
  $('#v011ClearSculpt')?.addEventListener('click', () => terrainSculptAction(object.id, 'clear'));`, 'app/v011.js');
  next = replaceRequired(next,
...
`  $('#v011ReversePath')?.addEventListener('click', () => pathAction(object.id, 'reverse'));
  ...
  $('#v011SplitPath')?.addEventListener('click', () => pathAction(object.id, 'split', { index: selectedIndex }));`, 'app/v011.js');
  if (!next.includes('async function terrainSculptAction')) {
    next = next.replace('async function updatePath(id, properties) {', `async function terrainSculptAction(id, action) {
  try {
...
`    if (action === 'split') { splineEditPathId = null; selectedSplineNodeIndex = null; }`, 'app/v011.js');
  next = replaceRequired(next,
`    if (!handle) {
      handle = document.createElement('button');`,
`    if (!handle) {
      handle = document.createElement('button');`, 'app/v011.js');
  next = replaceRequired(next,
`    handle.hidden = false;
...
`    handle.hidden = false;
    ...
    handle.style.transform = \`translate(${screen.x}px, ${screen.y}px)\`;
    handle.textContent = String(index + 1);`, 'app/v011.js');
  next = replaceRequired(next,
...
`  selectedSplineNodeIndex = Number(event.currentTarget.dataset.splineNode);
  ...
  enhanceInspector();`, 'app/v011.js');
  if (!next.includes("terrainSculptMode?.terrainId")) throw new Error('Terrain sculpt UI was not integrated.');
  next = replaceRequired(next,
...
`  canvas.addEventListener('click', async event => {
  ...
  canvas.addEventListener('mousedown', event => {
    if (!splineEditPathId) return;`, 'app/v011.js');
  return next;
});

edit('app/v011.css', source => appendOnce(source, '.v011-terrain-sculpting #viewport', `
...
uniform float uMoonIntensity;`, 'app/renderer.js');
  next = replaceRequired(next,
...
`  vec3 diffuse=uLightColor*ndl*uLightIntensity*shadow;
  ...
  vec3 halfDir=normalize(lightDir+viewDir);`, 'app/renderer.js');
  next = replaceRequired(next,
`  vec3 color=(baseLinear*(ambient+diffuse)+editorAmbient)*slopeCavity+uLightColor*spec*uLightIntensity;`,
`  vec3 color=(baseLinear*(ambient+diffuse+moonDiffuse)+editorAmbient)*slopeCavity+uLightColor*spec*uLightIntensity;`, 'app/renderer.js');
  next = replaceRequired(next,
`    const sun=scene.objects.find(o=>o.type==='directionalLight'&&o.visible);`,
`    const sun=scene.objects.find(o=>o.type==='directionalLight'&&o.visible&&o.properties?.celestialRole==='sun')||scene.objects.find(o=>o.type==='directionalLight'&&o.visible);`, 'app/renderer.js');
  next = replaceRequired(next,
...
`    lights.environment=environment;
    ...
    if(lights.shadows)this.renderShadow(scene,lightViewProj);`, 'app/renderer.js');
  next = replaceRequired(next,
...
`    if(scene.settings.splinesVisible!==false){const xray=typeof document!=='undefined'&&document.body.classList.contains('v011-spline-editing');if(xray)gl.disable(gl.DEPTH_TEST);for(const pathObject of scene.objects.filter(o=>o.type==='path'&&o.visible&&o.properties?.showSpline!==false)){const buffers=this.pathBuffers(pathObject,scene),selected=pathObject.id===selectedId;this.drawLines(buffers.edges,mat4Identity(),viewProj,selected?[.96,.56,1,1]:[.56,.34,.18,.7],selected?3:2);if(selected)this.drawLines(buffers.center,mat4Identity(),viewProj,[1,.9,1,1],3);}if(xray)gl.enable(gl.DEPTH_TEST);}`, 'app/renderer.js');
  return next;
});

console.log('Applied Phase 1.1 celestial, atmosphere, path, and terrain authoring corrections.');
