...
function bindEvents() {
  ...
  ui.newSceneButton.addEventListener('click',()=>ui.newSceneDialog.showModal());ui.newSceneTemplateInput?.addEventListener('change',()=>{if(ui.newSceneTemplateInput.value==='pulse-benchmark'&&(!ui.newSceneNameInput.value.trim()||ui.newSceneNameInput.value==='New Scene'))ui.newSceneNameInput.value='PULSE Foundry';});ui.viewportSettingsButton.addEventListener('click',()=>{ui.lookSensitivityInput.value=String(camera.lookSensitivity||.0023);ui.invertHorizontalInput.checked=Boolean(camera.invertHorizontal);ui.invertVerticalInput.checked=Boolean(camera.invertVertical);ui.moveSpeedInput.value=String(camera.moveSpeed||12);ui.fovInput.value=String(camera.fov||62);ui.helpDialog.showModal();});
