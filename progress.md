...
- Exact packaged pointer input moved a real node in X/Z, raised it 2.1 m, lowered it 3.6 m, and committed one revision per release. Civil Assist reclassified the affected construction, all generated meshes remained finite, and three live Undo actions restored the exact original coordinates and height mode with zero renderer or console errors.
- Visual inspection rejected that otherwise-valid run: the one-kilometre terrain used an 8.47 m base cell while the road was only 2 m wide, so per-vertex terrain modification and material interpolation expanded the bounded path field into broad dirt triangles and abrupt cliff walls.
- Terrain rendering now keeps untouched cells at their authored density while tessellating only a padded halo around terrain-modifying compiled path entries. The local patch resolves to approximately 0.75 m samples, snaps its outer transition vertices onto neighboring coarse edges, and continues to query the same signed-distance modifier used by collision, navigation, grounding, and foliage.
- Permanent regression coverage proves the v2 terrain patch gains local density, carries the authoritative height/material sample at the centerline, and prevents nonzero path material weights from escaping the corridor. The actual preserved target-PC package must be rebuilt and visually rechecked before this gate can pass.
