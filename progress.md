...
- The replacement renderer builds complete terrain chunks instead of floating local patches. Terrain-modifying path chunks plus a one-chunk halo use 0.75 m sampling; unchanged chunks retain the authored world density; dense-to-coarse edges are projected onto the exact neighboring coarse edge segments.
- The preserved one-kilometre target-PC fixture now builds 15 high-density construction chunks in about 1.3 s under the Node test runner. All indices, positions, normals, duplicate chunk-edge heights, material bounds, and the 5 mm boundary tolerance pass. Exact packaged multi-angle evidence remains mandatory, and synchronous generation time still exceeds the final 500 ms worker-pool target.
