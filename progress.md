...
- Path runtime caching now survives immutable server state replacement. A bounded stable-signature cache reuses unchanged compiled roads across cloned scene objects, while an edited road alone receives a new compiled runtime. Custom terrain-query services remain isolated unless callers explicitly opt into stable reuse.
- Path Network v2 spline handles are now real revisioned authoring data rather than dormant schema fields. The Inspector exposes automatic, aligned, and free modes plus compact 3D incoming/outgoing vectors; aligned edits preserve independent reach while enforcing exactly opposed directions.
- Manual handle edits use the same `/api/v012/path/*` transaction, monotonic revision, undo, redo, persistence, and compiler authority as node movement. Junction nodes remain automatic because one shared handle pair cannot unambiguously control three or more approaches.
- A live editor gate changed a middle node from automatic to free and moved its compiled corridor by 4.91 m, changed it to aligned with an opposed cosine of `-1`, undid, redid, and restored the original automatic state. The run recorded zero console/page errors and the inspected screenshot shows the simplified handle controls alongside the real compiled road.
- The handle gate is not final Path Network acceptance. Remaining work includes visible in-viewport handle gizmos, multi-node transactions, expanded construction heatmaps, semantic access/crossing constraints, full dirty-chunk worker cutover, legacy-runtime removal, exact packaged Windows interaction, and the required multi-angle construction-family captures.
- Exact packaged inspection uncovered a vertical-profile defect that the source-only gates had not exposed: an infeasible pair of hard endpoint elevations was clamped internally and then re-snapped at the endpoint, concentrating the unavoidable elevation change into one artificial sample and reporting a false 1,038% local cliff.
- Infeasible endpoint profiles now remain continuous and monotonic along cumulative arc length. The compiler reports the honest unavoidable route grade, withholds invalid construction output, and leaves the actionable failure visible without manufacturing a local discontinuity.
- A permanent compiler regression asserts that every station interval in an infeasible hard-anchor profile carries the same unavoidable grade. This is a corrective compiler gate; the exact rebuilt package still requires direct node dragging, vertical editing, undo/redo, and multi-angle inspection before acceptance.
- The first path-local adaptive-cell renderer was rejected from exact visible package evidence and reverted in commit `b8375e4`: independently refined cells produced missing triangles, overlapping brown terrain islands, and non-watertight vertical seams. None of that approach remains authoritative.
- The visual failure exposed a separate signed-distance root defect: endpoint-clamped samples used perpendicular distance instead of true closest-point distance, so the material and terrain field continued along segment tangent extensions until a chunk boundary. Segment influence now has bounded capsule end caps, with permanent before-start and after-end regressions.
- The replacement renderer builds complete terrain chunks instead of floating local patches. Terrain-modifying chunks use local high-density sampling; unchanged chunks retain the authored world density; dense-to-coarse edges are projected onto the exact neighboring coarse edge segments.
- The preserved one-kilometre target-PC fixture remains under the 5 mm boundary tolerance with finite positions, normals, and indices. Exact packaged multi-angle evidence remains mandatory, and synchronous generation time still exceeds the final 500 ms worker-pool target.
- Exact `bf5c148` packaged node manipulation passed horizontal drag, Shift-raise, Shift-lower, three-step undo, Save, finite mesh validation, and AMD RX 7900 XTX renderer diagnostics without WebGL loss. Seven-view inspection still blocked approval because coarse terrain material interpolation presented a saw-toothed dirt halo and player-level views did not clearly separate the authored road surface from its underlay.
- High-density work is now limited to modifier-owned chunks at 0.35 m spacing instead of expanding through a costly halo. The terrain underlay carries path material only beneath the road and shoulder; the compiled road/shoulder/earthwork surfaces own the visible boundary, including conforming paths. This gate must be rebuilt and visually compared before acceptance.
- Exact packaged `274db75` rejected that single-tier density change. Modifier chunks met coarse authored chunks abruptly, creating visible vertical chunk curtains, and the 0.35 m synchronous rebuild caused the real node-drag transaction to miss its 30-second acceptance timeout.
- The replacement source now uses three complete terrain tiers: 0.625 m modifier chunks, a one-chunk 2 m transition ring, and authored density elsewhere. Every denser boundary is projected onto the neighboring coarser edge, while signed-distance corridor sampling runs only in chunks conservatively intersecting a compiled modifier.
- The exact saved-project fixture generates 63,901 finite vertices and 117,062 finite triangles with zero measured shared-edge height mismatch. Warm isolated generation measured about 442-487 ms on this PC; all 235 source tests and repository verification pass. Exact packaged node manipulation and multi-angle visual acceptance remain required before this gate can be pushed or approved.
- Exact packaged `6aac943` records the real Git commit, opened on the RX 7900 XTX, and passed horizontal node movement, Shift-raise, Shift-lower, three revision-checked undos, exact coordinate restoration, and Save with no console/page error or WebGL context loss.
- Wide, close, left, rear, elevated, underside, and player-level captures prove the giant terrain islands, top-visible blue holes, and vertical chunk curtains are removed. The road remains continuous through every accepted edit, and the restored runtime reports finite, non-degenerate road, shoulder, earthwork, and structure meshes.
- Full Path Network acceptance remains blocked. Close and underside review still finds a rough outer earthwork/support transition, so the next gate must refine the shared construction boundary rather than treating this topology recovery as final visual approval.
- Exact packaged review then isolated a separate spline-compiler defect at true dead ends: the automatic tangent for a segment's terminal node pointed from the endpoint back toward its neighbor. Hermite evaluation therefore overshot the authored endpoint, reversed direction, and folded the wide earthwork sweep over itself even though every triangle remained finite and non-degenerate.
- Degree-one terminal tangents now point into the endpoint along the segment travel direction. On the exact saved route, terminal-segment maximum curvature fell from about `6.9195` (approximately `0.14 m` radius) to `0.0571`, and sampled X positions approach the endpoint monotonically without passing it.
- Commit `e81dbe3c22bf403e4c307b353b785c41df156ce5` passed all 236 source tests and repository verification, rebuilt the Windows package with an exact matching `source-commit`, and ran in the visible desktop application on this PC.
- Actual packaged pointer input moved a node horizontally, Shift-raised it, Shift-lowered it, undid all three edits through monotonically increasing revisions `41` through `47`, restored the exact original coordinates and height mode, and saved. No console errors, page errors, WebGL loss, non-finite geometry, invalid indices, or degenerate triangles were recorded.
- Clean packaged wide, close, left, rear, elevated, player-level, and both dead-end views show a continuous bounded road with no former inner-bend spur, loopback, dirt island, or top-visible sky hole. A separate real Shift-drag stress gate raised and lowered a middle node by `12 m`; Civil Assist selected bridge/tunnel modes and marked the unavoidable adjacent grade invalid rather than claiming the route was traversable. Two undos restored the exact original graph.
- Full Path Network acceptance remains blocked. Below-terrain inspection still exposes the thin terrain sheet and an open construction/terrain underside, extreme invalid routes need clearer persistent visual diagnostics, and bridge/tunnel/retaining-wall/stair families still require dedicated exact packaged scenes and close/side/rear/underside approval.
- Exact packaged `7bfb6a7` proves Civil Assist now classifies construction per local interval: the 427 m dirt route follows terrain without the former full-route bridge or endless pier field, and a short local depression no longer promotes an entire segment into a bridge.
- The separate `Terrain Path Branch` is a second Path Network object rather than a connected branch. A new atomic merge transaction is in progress: it inserts a true degree-three junction on the nearest compiled target segment, imports the branch graph, removes the separate scene object, and makes Undo/Redo restore or remove that object as one edit.
- Visible packaged testing of the first merge exposed a nearly coincident connector and a self-intersecting junction. Branch joins now weld the source endpoint directly into the target topology, snap exact endpoint joins without creating zero-length segments, and use a bounded non-radial convex cleanup only for valid, clearly separated approaches. The exact target-PC fixture and the current recovered v2 network now produce finite junction geometry without self-intersection.
- Current bridge/terrain gate replaces the generic two-girder/endless-gray-pier output with five data-driven structural families: timber trestle, stone arch, steel girder, masonry causeway, and pedestrian rope footbridge. Automatic selection uses compiled local span, width, clearance, and vehicle class; manual style selection is a revisioned Path Network transaction.
- Terrain-following dirt roads now have a permanent geometry regression proving they emit no bridge roles. Only compiler-classified local bridge intervals can construct a bridge.
- Bridge structures preserve material groups with timber, masonry, concrete, steel, and rope PBR response and are submitted through the world shadow-receiving/casting path.
- Partially invalid merged networks retain exact red diagnostic guides while valid segments still produce road, collision, and navigation geometry. Invalid branches no longer make the complete connected road disappear.
- Focused path construction/model/renderer suite: 24/24 passed. Full repository tests, exact packaged build, and multi-angle visual approval are still pending; no final visual claim has been made.
- Exact packaged multi-angle review rejected the first steel bridge output: automatic family selection was correct, but the bridge approaches lacked abutments and a single narrow rectangular column read as an unfinished support.
- Steel bridges now use multiple longitudinal girders, grounded footings, paired flared pier columns, full-width cap beams, backwalls, and wingwalls. All five structural families receive grounded endpoint abutments; this corrective gate remains blocked pending rebuilt-package visual approval.
- Rebuilt package `7dcdfce` remained visually blocked on the exact 60 m / 26.7 m-deep target-PC gap: endpoint topology existed but did not read clearly and the height-independent pier proportions were too slender. The next corrective slice scales paired columns and cap beams by support height, adds bracing to tall piers, and strengthens the abutment footprint before another exact-package review.
- Rebuilt package `599d125` proved the deep supports at side and underside angles, but its approach capture exposed an abutment anchor defect: the minimum backwall height extended upward from shallow bank terrain and could rise above the deck. Abutment seats are now deck-anchored and all extra backwall/footing depth extends downward into the bank.
- Package `94325e9` removed the above-deck backwall, but the approach capture still exposed the broad footing top by a few centimetres on the irregular bank. Footings are now deliberately buried beneath both terrain and backwall bottom; only the deck seat, backwall face, and wingwalls remain visible.
- Real packaged node-drag testing exposed two synchronous input costs: terrain picking scanned up to 12 km while sampling compiled path modifiers, and every pointer move replaced the renderer path preview, forcing a full terrain/corridor/structure rebuild. Terrain picks are now clipped to the terrain bounds and use the authored base surface for editing; drag preview moves the node overlay without replacing runtime authority, and one connected-system rebuild occurs on release.
- Exact package input probing then found that Chromium delivered a valid primary pointer event to the visible spline handle while the per-handle editor listener did not start a drag transaction. Node input is now delegated once through the stable overlay, so continuously reconciled handle elements cannot lose the editor interaction authority. Rejected primary-pointer guards emit explicit diagnostics.
- The remaining intermittent miss was a CSS interaction defect rather than terrain generation: standalone `scale: 1.12` on hover also scaled each handle's large inline translation, moving the target about 185 px away from the pointer and back every hover cycle. Hover feedback now changes brightness, color, and shadow without moving the authoring target. Live style injection proved horizontal drag and Shift-raise both commit one revision once the target stays stable.
- The current dirt-path failure was traced to three connected source defects rather than a missing cosmetic mesh: per-station Civil Assist thresholds produced sub-metre conform/cut-fill/bridge chatter, automatic bridge profiles inherited the legacy generic 8 m support spacing instead of their family spacing, and the terrain stitch pass used one segment-wide representative mode that could suppress stitches along valid dirt intervals whenever another interval was structural.
- Construction intervals now use width-aware hysteresis and minimum run lengths. Isolated short depressions become continuous cut/fill earthwork instead of tiny bridges or pier fields, while genuine adjacent bridge runs can join across a small terrain lip and still respect the maximum span.
- Automatic bridge families recover their authored structural spacing (`24 m` for the steel-girder fixture) unless the user explicitly authored a non-legacy override. Bridge diagnostics now record the resolved spacing used by geometry.
- Terrain boundary stitches are evaluated from each compiled cross-section's local construction authority. A bridge or tunnel interval no longer disables terrain/road boundary closure elsewhere on the same segment.
- The exact target-PC fixture now compiles without any terrain interval shorter than `3.8 m`; the isolated 2 m depression emits no bridge selection or support role. All `260` source tests and repository verification pass. The exact rebuilt Windows package and actual multi-angle interaction/visual pass remain mandatory before this gate can be approved.
- Exact packaged close inspection of `598d765` found a separate surface-ownership defect: `4,430 / 4,713` road vertices were coplanar with the path-modified terrain. The resulting depth conflict made an otherwise continuous compiled dirt surface intermittently disappear or look buried.
- Cross-section profiles now carry a bounded `terrainUnderlayClearance` (default `0.04 m`). The authored road target remains the collision/navigation/grounding surface, while supporting terrain is lowered inside the same cut budget and the separation feathers to zero at the unchanged outer boundary. A permanent regression proves road and shoulder ownership separation without exceeding cut/fill limits.
- This corrective underlay gate passes the focused terrain/geometry/target-PC suite. It is not visually approved until a new exact-commit Windows package repeats the close, side, opposite, elevated, node-edit, and persistence captures.
- Exact packaged bridge inspection exposed a second surface-authority collision: ordinary dirt road and shoulder strips were generated across every segment before the bridge deck was added at the same stations. The overlapping coplanar surfaces made bridge materials flicker and left dirt visibly suspended through structural spans.
- Visible path surfaces now compile per Civil Assist interval. Conform, cut/fill, retaining-wall, and tunnel intervals own the road and shoulder; stairs own their treads and risers; bridge intervals are owned exclusively by their selected structural deck. Adjacent interval endpoints share the same compiled station positions.
- Stone-arch spring points are held below the deck and use the common grounded abutment assembly, eliminating collapsed spandrel triangles and duplicate endpoint masses. A mixed terrain/bridge regression proves no ordinary road vertices remain inside the bridge interval.
- All `262` source tests and repository verification pass. Exact rebuilt-package bridge lighting, shadow, approach, underside, and node-edit evidence is still required; the branch remains visually blocked.
- Exact packaged bridge review then confirmed that structural roles used only scalar colors and the grouped-material renderer loaded only base color, even though the shared PBR shader supports normal, roughness, and AO maps.
- Four production CC0 Poly Haven material families are now vendored with provenance and checksums for timber, masonry, concrete, and steel structures. Bridge groups expose all four PBR channels, non-terrain grouped models receive normal mapping, and structural box UVs repeat by physical face size instead of stretching one texel set across an entire pier or girder.
- This material/lighting gate remains unapproved until the exact committed package is captured from bridge approach, side, rear, elevated, and underside angles with visible sunlight and shadow response.
- Exact packaged review of `8392d52a8009f8fa75f333dd7d791d0609437149` loaded the production PBR bridge maps but rejected the steel-girder structure from approach, side, rear, elevated, and underside views. The deck underside and several oriented structural faces were folded or missing, and the far abutment appeared as a detached pile rather than a grounded portal.
- Root cause was not the material pass: the shared triangle helper forcibly reversed every downward-facing triangle. That corrupted legitimate bridge undersides, beam bottoms, box side topology, shadow winding, and any sloped face whose normal happened to point below the horizon. A second defect projected world-height differences through the road's sloped local up vector, moving abutment foundations sideways away from their approach portals.
- Triangle winding is now preserved as authored. Bridge deck top, underside, edges, end caps, shoulders, and oriented structural boxes emit explicit outward counter-clockwise faces. Abutment footings, backwalls, and wingwalls use horizontal portal frames with world-up height placement and bounded terrain sampling so elevation cannot produce lateral drift.
- Permanent regressions verify upward deck tops, downward deck undersides, complete girder face directions, finite abutment geometry, and bounded portal placement. Focused construction/geometry tests pass `18/18`; the complete `267`-test suite and repository verification pass. This corrective topology gate remains visually blocked until the newly committed Windows package repeats the same exact multi-angle inspection.
- Exact Windows package `99f04afc75ec8246e0f6da2aa7956a345f97d4a0` embeds the matching real Git commit and passed the corrective approach, side, opposite, elevated, underside, and grounded player-level topology review. The formerly folded underside is closed and outward-facing, oriented supports retain complete faces, and both abutments remain at their portals; no page error, WebGL context loss, or new renderer error occurred.
- Actual packaged input moved a bridge endpoint horizontally, Shift-raised it by `4.5 m`, rebuilt the connected bridge, then undid both operations through revisions `5` to `8` and restored the exact original coordinates. A separate gentle automatic dirt-route fixture followed the terrain with no structural supports or bridge surface.
- This passes only the rejected solid-topology correction. Full Path Network approval remains `BLOCKED`: dirt-to-deck grading still needs dedicated transition refinement, every structural family needs its own span-appropriate packaged scene, branch merge needs packaged interaction proof, and aggressive terrain edits still require close/side/underside seam and hole inspection.
- The first grounded player-level bridge capture exposed a direct boundary-authority mismatch: normal roads used a three-point crowned cross-section while bridge decks used a flat two-point strip. Their outer edges met, but the missing shared crown produced a visible wedge and shading seam at each dirt-to-deck transition.
- `CompiledPathNetwork` terrain cross-sections now publish the same `roadCenter` crown vertex consumed by both ordinary road and structural bridge geometry. Bridge deck tops, undersides, and end caps preserve that three-point profile, so road and deck share exact left/crown/right boundary positions instead of approximating the transition independently.
- A permanent mixed-interval regression compares the actual road-core and bridge-deck boundary vertices at both portals and requires exact five-decimal equality. All `267` source tests and repository verification pass; exact rebuilt-package approach, player-level, side, elevated, underside, node-edit, and undo evidence remains mandatory before this transition gate can be approved.
